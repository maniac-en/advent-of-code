...
while q:
    ...
    if r < len(grid) - 1 and ch in "S|7F" and grid[r + 1][c] in "|JL" and\
            (r + 1, c) not in loop:
        loop.add((r + 1, c))
        q.append((r + 1, c))
        if ch == "S":
    ...
    if c < len(grid[r]) - 1 and ch in "S-LF" and\
            grid[r][c + 1] in "-J7" and (r, c + 1) not in loop:
        loop.add((r, c + 1))
        q.append((r, c + 1))
        if ch == "S":
            maybe_s &= {"-", "L", "F"}
...
grid = [
    "".join(
        ch if (
            r,
            c) in loop else "." for c,
        ch in enumerate(row)) for r,
